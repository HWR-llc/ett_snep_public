    ...
    <div class="row align-items-center">
      ...
      <div class="col-10">
        <div class="row">
          ...
          <div class="col-3">
            <div class="form-check" v-for="selection in waterQualitySelections.slice(6,8)" :key="selection.id">
              <input class="form-check-input" type="radio" :id="selection.id" :value="selection.id" v-model="waterQuality">
          ...
          <div class="col-3">
            <div class="form-check" v-for="selection in waterQualitySelections.slice(8,10)" :key="selection.id">
              <input class="form-check-input" type="radio" :id="selection.id" :value="selection.id" v-model="waterQuality">
              <label class="form-check-label" :for="selection.id" style="color: white"> {{selection.capital}} </label>
            </div>
          </div> -->
          <div class="col-md-auto">
            <div>
              <b-form-select v-model="waterQuality" :options="wqOptions"></b-form-select>
            </div>
          </div>
        </div>
...
export default {
  ...
  computed: {
    // computed property to generate dropdown options in required format
    wqOptions: function () {
      var options = [{value: null, text: 'no parameter selected'}];
      waterQualitySelections.forEach(function (wq) {
        options.push({value: wq.id, text: wq.capital})
      });
      return options;     
    },
    pointsLayer: {
